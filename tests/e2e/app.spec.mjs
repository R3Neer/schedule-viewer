...
async function navigateSection(page, name) {
  while ((await page.locator("#settings-dialog").getAttribute("data-panel")) !== "home") {
...
async function openSettings(page, tab = "Horario") {
  await page.mouse.click(20, 300);
  await page.locator("#settings-button").click();
  await expect(page.locator("#settings-dialog")).toHaveAttribute("data-motion-state", "open");
  if (tab) await navigateSection(page, tab);
}
...
async function chooseDefaultImage(page) {
  ...
  await page.locator('[data-image-key="default"]').getByRole("button", { name: "Cambiar" }).click();
...
async function waitForServiceWorker(page) {
  ...
  await expect.poll(() => page.evaluate(async () => (await caches.keys()).some((key) => key === "schedule-viewer-offline-20260904-v4-18"))).toBe(true);
}
...
test.describe("iPhone / Apple", () => {
  ...
  test("demo diaria, Liquid-Glass web y aviso de personalización", async ({ page }) => {
    ...
    await expect(page.locator("#demo-hint")).not.toHaveAttribute("hidden", "");
    await expect(page.locator("#settings-button > svg.settings-gear-icon")).toHaveCount(1);
    await expect(page.locator("#demo-hint > svg.settings-gear-icon")).toHaveCount(1);
    expect(await page.locator("#settings-button").textContent()).not.toContain("⚙");
    await expect.poll(() => page.locator("#settings-button").evaluate((node) => {
      const render = node.dataset.liquidGlassRender || "";
      return ["webgl", "svg", "frost"].includes(render) || getComputedStyle(node).backdropFilter.includes("blur");
    })).toBe(true);
  ...
  test(".schedule restaura configuración + assets después de Restablecer demo", async ({ page }) => {
    ...
    await chooseDefaultImage(page);
    await saveSettings(page);
    await navigateSection(page, "Copia de seguridad");
...
test.describe("Escritorio", () => {
  ...
  test("Ctrl+, abre, Escape cierra y desactivar Space persiste", async ({ page }) => {
    ...
    await navigateSection(page, "Horario");
