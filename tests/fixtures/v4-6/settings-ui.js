...
export function initSettingsUI({ deviceMode, getState, applyLocalConfig, resetToDemo }) {
  ...
  const showPanel = (name) => {
    const titles = { home: "Ajustes", schedule: "Horario", images: "Imágenes", backup: "Copia de seguridad", advanced: "Avanzado" };
    const title = document.querySelector("#settings-title");
    title.textContent = titles[name] || "Ajustes";
    document.querySelector("#settings-back").hidden = name === "home";
  ...
  document.querySelector("#settings-back").addEventListener("click", () => {
    ...
    showPanel("home");
    dialog.querySelector(`[data-settings-tab="${previous}"]`)?.focus({ preventScroll: true });
  });
